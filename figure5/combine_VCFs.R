...
VCFfiles <- list.files(path="/zfstank/ngsdata/projects/icell8_cellenion_publication/figure5/target_variants", pattern = "_variants_filt.vcf$", full.names = T)
...
vcf <- lapply(1:length(VCFfiles), function(i){
  ...
  tmp <- Reduce("rbind",geno(vcf)$AD[,1])
...
# Remove variations in control (GOE1303 and GOE1305) but no in disease patients
...
df$genes <- sapply(1:nrow(df),function(i){
  values <- list(chromosome_name=gsub("chr","",gsub("chrM","chrMT",df$chr[i])),start=df$pos[i],end=df$pos[i])
  paste(unique(getBM(attributes=attributes, filters=filters, values=values, mart=mart)),collapse=",")
})

write.table(df,file="/zfstank/ngsdata/projects/icell8_cellenion_publication/figure5/target_variants/VCF_annotations.csv",
