...
## Icell8 
icell8_mat <- icell8$qc_data$metadata[,c("Sample","Barcoded_Reads","Exon_Reads","No_of_Genes")]
icell8_norm <- icell8$qc_data$gm
icell8_mat$No_of_Genes_CPM01 <- apply(icell8_norm, 2, function(x) sum(x>0.1))
icell8_mat$No_of_Genes_CPM1 <- apply(icell8_norm, 2, function(x) sum(x>1))
icell8_mat$platform="ICELL8"
icell8_mat <- icell8_mat[!(icell8_mat$Sample %in% c("Pos_Ctrl","Neg_Ctrl")),]
icell8_mat$Sample <- tolower(icell8_mat$Sample)
## Cellenion 
cellenion_mat <- composite$qc_data$metadata[,c("Sample","Barcoded_Reads","Exon_Reads","No_of_Genes")]
cellenion_norm <- composite$qc_data$gm
cellenion_mat$No_of_Genes_CPM01 <- apply(cellenion_norm, 2, function(x) sum(x>0.1))
cellenion_mat$No_of_Genes_CPM1 <- apply(cellenion_norm, 2, function(x) sum(x>1))
cellenion_mat$platform="CellenONE"
cellenion_mat <- cellenion_mat[!(cellenion_mat$Sample %in% "Goe247_wo__fix"),]
cellenion_mat$Sample <- tolower(cellenion_mat$Sample)
# Plot 
...
ggplot(plotDF[plotDF$variable=="No_of_Genes",], aes(factor(platform), value, fill=platform))+
  ...
  labs(title="Number of genes expressed in cells (read count >0)", x="Platform", y="#Genes")
